...
void vkglTF::Texture::destroy()
{
	if (device)
	{
		...
		vkFreeMemory(device->m_logicalDevice, deviceMemory, nullptr);
		vkDestroySampler(device->m_logicalDevice, sampler, nullptr);
	}
}
...
void vkglTF::Texture::fromglTfImage(tinygltf::Image& gltfimage, std::string path, VkSandboxDevice* device, VkQueue copyQueue, bool isSrgb)
{
	...
	if (!isKtx) {
		...
		vkGetImageMemoryRequirements(device->m_logicalDevice, image, &memReqs);
		memAllocInfo.allocationSize = memReqs.size;
		memAllocInfo.memoryTypeIndex = device->getMemoryType(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
		VK_CHECK_RESULT(vkAllocateMemory(device->m_logicalDevice, &memAllocInfo, nullptr, &deviceMemory));
		VK_CHECK_RESULT(vkBindImageMemory(device->m_logicalDevice, image, deviceMemory, 0));
	...
	VK_CHECK_RESULT(vkCreateSampler(device->m_logicalDevice, &samplerInfo, nullptr, &sampler));
	...
	VK_CHECK_RESULT(vkCreateImageView(device->m_logicalDevice, &viewInfo, nullptr, &view));
...
vkglTF::Mesh::Mesh(VkSandboxDevice* device, glm::mat4 matrix) {
	...
	VK_CHECK_RESULT(device->createBuffer(
		...
		&uniformBlock));
	VK_CHECK_RESULT(vkMapMemory(device->m_logicalDevice, uniformBuffer.memory, 0, sizeof(uniformBlock), 0, &uniformBuffer.mapped));
...
vkglTF::Model::~Model()
{
	...
	vkDestroyBuffer(m_pDevice->m_logicalDevice, indices.buffer, nullptr);
	vkFreeMemory(m_pDevice->m_logicalDevice, indices.memory, nullptr);
...
void vkglTF::Model::loadMaterials(tinygltf::Model& gltfModel)
{
	...
	m_materials.push_back(Material(m_pDevice));
}
...
void  vkglTF::Model::loadFromFile(std::string filename, VkSandboxDevice* device, VkQueue transferQueue, uint32_t fileLoadingFlags, float scale)
{
	...
	VK_CHECK_RESULT(vkCreateDescriptorPool(device->m_logicalDevice, &descriptorPoolCI, nullptr, &m_descriptorPool));
	...
	{
		{
			...
			if (descriptorSetLayoutUbo == VK_NULL_HANDLE) {
				...
				VK_CHECK_RESULT(vkCreateDescriptorSetLayout(device->m_logicalDevice, &descriptorLayoutCI, nullptr, &descriptorSetLayoutUbo));
			}
			for (auto node : m_nodes) {
